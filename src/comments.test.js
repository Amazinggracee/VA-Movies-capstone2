...
describe('correct commentCounter', () => {
  beforeEach(() => {
  ...
  test('correct count on user comments', () => {
    ...
    expect(commentCounter()).not.toBe(true);

    document.body.innerHTML =
      '<div class="comments"><div>comment 1</div><div>comment 2</div></div>';
  ...
  test('returns 0 when passed undefined as an argument', () => {
    const result = commentsCounter(undefined);
    expect(result).toBe(0);
  });
});
